...
class Model {
  ...
  private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene) {
        ...
        loadTextureMaterial(
            material, aiTextureType_DIFFUSE, "texture_diffuse", textures);

        loadTextureMaterial(
            material, aiTextureType_SPECULAR, "texture_specular", textures);

        loadTextureMaterial(
            material, aiTextureType_NORMALS, "texture_normal", textures);

        loadTextureMaterial(
            material, aiTextureType_HEIGHT, "texture_height", textures);
    ...
    void loadTextureMaterial(
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); ++i) {
            ...
            for (unsigned int j = 0; j < loaded_textures.size(); ++j) {
                if (std::strcmp(str.C_Str(), loaded_textures[j].path.c_str()) ==
                    0) {
