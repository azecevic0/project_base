...
enum VampireState { APPROACHING, ROTATING, AWAY };
...
class Vampire {
  ...
  public:
    ...
    void draw(Shader &shader, const float frameTime, const float delta) {
        ...
        while (!m_garlicTime.empty() &&
               frameTime - m_garlicTime.front() > 3.0f) {
    ...
    void attack(
        ...
        const float time) {
        auto planarDirection =
            10.0f * glm::normalize(glm::vec2(direction.x, direction.z));
        glm::vec3 garlicPosition { position.x + planarDirection.x, 2.2f,
                                   position.z + planarDirection.y };
  ...
  private:
    ...
    void setupVampireModelMatrix() {
        ...
        m_vampireModelMatrix =
            glm::translate(m_vampireModelMatrix, m_vampirePosition);
        m_vampireModelMatrix = glm::rotate(
            m_vampireModelMatrix, glm::radians(m_angle), glm::vec3(0, 1, 0));
        m_vampireModelMatrix =
            glm::scale(m_vampireModelMatrix, glm::vec3(VAMPIRE_SCALE));
    }
    ...
    bool isAttacked() {
        ...
        for (const auto &pos : m_garlicPosition) {
            if ((pos.x - x) * (pos.x - x) + (pos.z - z) * (pos.z - z) <
                100.0f) {
                return true;
            }
        }
